...
describe 'return_stdout_lines against multiline output' do
  context command('ls -al /') do
    let(:stdout) { <<EOF
total 88
...
EOF
    ...
    it { should     return_stdout_lines(/root/) }
  end
end

describe 'return_stdout against multiline output' do
  context command('ls -al /') do
    let(:stdout) { <<EOF
total 88
drwxr-xr-x  23 root root  4096 Oct 10 17:19 .
